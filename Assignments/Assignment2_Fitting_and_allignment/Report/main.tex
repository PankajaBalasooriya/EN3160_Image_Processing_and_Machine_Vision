...
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{resources/sunflowe_blobs.png}
    \caption{Detected circles in sunflower field image}
    \label{fig:sunflowers}
\end{figure}

\textbf{Largest Circles:}
\begin{itemize}
    \item Circle 1: Center $(282.00,	338.00)$, Radius $10.0$, $\sigma$ 7.17
    \item Circle 2: Center $(0.00,275.00)$, Radius $10.0$, $\sigma$ 7.17
    \item Circle 3: Center $(106.00,255.00)$, Radius $8.00$, $\sigma$ 5.97
\end{itemize}
...
Line parameterization: $(a, b, d)$ where $\|[a, b]^T\| = 1$
...
def ransac_line(points, iterations, threshold):
    ...
    for i in range(iterations):
        ...
        # Enforce unit normal constraint
        # Count inliers
        # Update best model
    return best_params, inliers
\end{lstlisting}

\textbf{Parameters:}
\begin{itemize}
    \item Iterations: N
    \item Distance threshold: $\epsilon$
    \item Consensus size: M points
\end{itemize}

\subsubsection{Circle Fitting with RANSAC}
Circle parameterization: $(x_0, y_0, r)$

After removing line inliers, fit circle to remaining points using:
\begin{itemize}
    \item Sample size: 3 points
    \item Error metric: Radial distance
    \item Threshold: $\epsilon_{circle}$
\end{itemize}

\subsection{Results}
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{ransac_fitting.png}
...
\begin{itemize}
    \item Line: $a=\ldots, b=\ldots, d=\ldots$
    \item Circle: $x_0=\ldots, y_0=\ldots, r=\ldots$
    \item Line inliers: XX points
    \item Circle inliers: YY points
\end{itemize}

\subsection{Discussion}
\textbf{Fitting circle first:} If the circle is fitted first, it may incorrectly capture some line points as inliers due to the larger number of circle points or if the circle passes near the line. This would leave insufficient points for accurate line estimation and reduce overall fitting quality.
